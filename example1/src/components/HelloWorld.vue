<template>
  <div class="content-wrapper">
    <multi-tab ref="multiTab" :bounce="false" :pullUpLoad="true" :initialIndex="9" @wrapperScroll="handleScroll" @scrollEnd="onScrollEnd">
      ...
      <!-- 默认区域 -->
      <!-- 设置每一页的内容自定义 -->
      <multi-tab-item v-for="i in 10" :key="i" :title="`demo${i}`">
        <first-demo v-if="i % 3 == 1" />
        <second-demo v-if="i % 3 == 2" />
        <third-demo v-if="i % 3 == 0" />
      </multi-tab-item>
    </multi-tab>
  </div>
</template>
...
import SecondDemo from '@/components/demo/SecondDemo'
import ThirdDemo from '@/components/demo/ThirdDemo'
export default {
  name: 'HelloWorld',
  components: {
    // Test
